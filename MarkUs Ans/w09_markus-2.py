...
#      w09_markus.py tab indicates that modifications have NOT been saved.
...
#
# This problem is based on the Structured Files exercise from Week 8 Lecture,
...
#    values list.
#
# Hint: Start with your code from the Week 8 points_per_game function.
...
'Montreal Canadiens': [1, 2, 1, 0, 2]}
    ...
    for info in game_data:
        info = info.strip()  
        if info.isdigit(): 
            game_dictionary[current_team].append(int(info))
        else: 
            current_team = info
            if current_team not in game_dictionary:
                game_dictionary[current_team] = []
