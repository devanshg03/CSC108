...
def build_dictionaries(f: TextIO
    ...
    for line in f:
        ...
        station_id_number = int(elements[0])
        ...
        station_mapping[station_id_number] = [latitude, longitude]
        
        id_to_name[station_id_number] = name
    
    return (station_mapping, id_to_name)
...
def update_amounts(totals: dict[str, int], amts: list[tuple[str, int]]) -> None:
    ...
    for item in amts:
        if item[0] in totals.keys():
            totals[item[0]] = totals[item[0]] + item[1]
        else:
